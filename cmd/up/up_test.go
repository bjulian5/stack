	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				...
				// Move to the change first (since up requires being on a UUID branch)
		...
		},
		{
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				...
				require.NoError(t, err)
				// When there's only one change, the UUID branch is TOP
				expectedBranch := "test-user/stack-test-stack/TOP"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
		{
			desc: "already at top with multiple changes warns and no-ops",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				currentBranch, err := gitClient.GetCurrentBranch()
				require.NoError(t, err)
				// Should stay on TOP (since checking out the last change keeps you on TOP)
				expectedBranch := "test-user/stack-test-stack/TOP"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
		{
			desc: "from bottom with two changes moves up to second",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				currentBranch, err := gitClient.GetCurrentBranch()
				require.NoError(t, err)
				// Should move to TOP (since second change is the top in a 2-change stack)
				expectedBranch := "test-user/stack-test-stack/TOP"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
		{
			desc: "from bottom with three changes moves up to second",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
		...
		{
			desc: "from middle position moves up one",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				currentBranch, err := gitClient.GetCurrentBranch()
				require.NoError(t, err)
				// Should move to TOP (since third change is the top in a 3-change stack)
				expectedBranch := "test-user/stack-test-stack/TOP"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
		{
			desc: "from second position in four-change stack moves to third",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
				...
				// Move to second change (position 2)
				stackCtx, err := stackClient.GetStackContext()
				require.NoError(t, err)
				_, err = stackClient.CheckoutChangeForEditing(stackCtx, stackCtx.ActiveChanges[1])
				require.NoError(t, err)
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				currentBranch, err := gitClient.GetCurrentBranch()
				require.NoError(t, err)
				expectedBranch := "test-user/stack-test-stack/3333333333333333"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
		{
			desc: "from third position in four-change stack moves to top",
			setup: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client, stackClient *stack.Client) {
				ghClient.On("GetRepoInfo").Return("test-owner", "test-repo", nil)
			...
			verify: func(t *testing.T, ghClient *gh.MockGithubClient, gitClient *git.Client) {
				currentBranch, err := gitClient.GetCurrentBranch()
				require.NoError(t, err)
				// Should move to TOP (since fourth change is the top)
				expectedBranch := "test-user/stack-test-stack/TOP"
				assert.Equal(t, expectedBranch, currentBranch)
			},
		},
